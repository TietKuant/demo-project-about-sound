...
PAIRED_COLUMNS = ["sample_id", "noisy_path", "clean_path", "split"]
CLEAN_SPEECH_COLUMNS = ["sample_id", "path", "split", "notes"]
VOICEBANK_CLEAN_NOTES = "VoiceBank-DEMAND clean speech source"
...
def _manifest_relative_path(path: Path, output_path: Path) -> str:
    return Path(os.path.relpath(path.resolve(), Path(output_path).resolve().parent)).as_posix()
...
def build_voicebank_manifest(
    *,
    ...
    if manifest_type == "paired":
        ...
        fieldnames = PAIRED_COLUMNS
        empty_message = f"No clean/noisy VoiceBank pairs found for split: {split}"
    elif manifest_type == "clean-speech":
        if not clean_dir.is_dir():
            raise ValueError(f"Expected clean folder is missing: {clean_dir}")
        for clean_path in sorted(clean_dir.glob("*.wav"), key=lambda path: path.name):
            rows.append(
                {
                    "sample_id": clean_path.stem,
                    "path": _manifest_relative_path(clean_path, output_path),
                    "split": split,
                    "notes": VOICEBANK_CLEAN_NOTES,
                }
            )
        fieldnames = CLEAN_SPEECH_COLUMNS
        empty_message = f"No clean VoiceBank speech files found for split: {split}"
    else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--manifest-type", choices=("paired", "clean-speech"), default="paired")
    return parser
