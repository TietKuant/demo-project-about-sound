...
from scripts.run_voicebank_benchmark import run_voicebank_benchmark
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("scripts.run_voicebank_benchmark.run_pipeline", side_effect=mock_pipeline):
                run_voicebank_benchmark(
                    ...
                    output_path=output_path,
                    output_root=root / "outputs",
                    engines=["ffmpeg-arnndn", "noisereduce", "deepfilternet"],
                )
            rows = self._read_rows(output_path)

        self.assertEqual(calls, ["ffmpeg-arnndn", "noisereduce", "deepfilternet"])
        self.assertEqual([row["status"] for row in rows], ["success", "failed", "success"])
        self.assertIn("engine failed", rows[1]["error"])
